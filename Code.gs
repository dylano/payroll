...
function runPayroll() {
  ...
  // Copy pay data from Template to new sheet  (values only)
...
function validateInput() {
  ...
  if(hrsReg + hrsOT <= 0) {
    Browser.msgBox('Error', 'Work harder.', Browser.Buttons.OK);
    return 0;
  }
  ...
  if(taxFed <= 0 || taxCa <= 0) {
    Browser.msgBox('Error', 'Pay your fair share.', Browser.Buttons.OK);
    return 0;
  }
